...
class NyansCog(commands.Cog):
    ...
    @commands.hybrid_command(name="roulette", description="5🐱を消費しルーレットを引きます")
    async def roulette(self, ctx: commands.Context):
        ...
        if row["nyans"] < -100:
            embed = discord.Embed(title="借金のし過ぎです！！", description="精算し、5分のタイムアウトを開始します。", colour=discord.Colour.red())
            ...
            await Database.pool.execute(
                ...
                """,
                ctx.author.id, row["nyans"],
            )
            return
        elif row["nyans"] < 5:
            embed = discord.Embed(title="🐱が足りません！", description="借金を開始 または 利息が増えます", colour=discord.Colour.red())
            await ctx.reply(embed=embed)
            row["nyans"] -= 2

        amount = random.randint(random.randint(-4, -1), random.randint(0, 7))
        row["nyans"] += amount
